...
private:
	...
	public:
		...
		std::optional<YulName> operator()(YulName _variable) const;
		/// @returns a YulName containing the memory offset to be assigned to @a _variable as number literal
		/// or std::nullopt if the variable should not be moved.
		std::optional<YulName> operator()(TypedName const& _variable) const;
		/// @returns a YulName containing the memory offset to be assigned to @a _variable as number literal
		/// or std::nullopt if the variable should not be moved.
		std::optional<YulName> operator()(Identifier const& _variable) const;
	...
	StackToMemoryMover(
		...
		std::map<YulName, std::vector<TypedName>> _functionReturnVariables
	);
